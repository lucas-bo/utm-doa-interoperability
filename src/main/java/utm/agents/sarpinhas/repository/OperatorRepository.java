package utm.agents.sarpinhas.repository;

import java.sql.Connection;
import java.sql.Statement;

import lombok.extern.log4j.Log4j2;
import utm.agents.sarpinhas.conn.ConnectionFactory;
...
public class OperatorRepository {
    public static void save(Operator operator){
        String sql = """
                    INSERT INTO sarpinhas.operator (name, cpf, email) 
                    VALUES ('%s',%d,'%s')"""
    ...
    public static void delete(int id){
        String sql = """
                    DELETE FROM sarpinhas.operator 
                    WHERE (id = '%d')"""
                        .formatted(id);
        try 
        (
